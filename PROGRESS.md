...
- [x] `apply` réel *(2026-08-17)* : transaction SQL ; vérifications **même après approbation** :
  ...
  - [x] ⬅️ *ajouté le 2026-08-17 (ADR 010, décision 14)* : le **seul DDL autorisé** est la restauration
        ...
        contenu.
  - [x] ⭐ **L'autorisation DDL ne se donne pas parce que le SQL en a la forme, mais parce qu'un FAIT
        constaté l'appelle** : un écart de famille `inventaire`. Sans cette condition, il suffirait au
        modèle d'écrire un renommage pour franchir un garde-fou qui existe précisément pour l'en
        empêcher. La cible est vérifiée en plus — elle doit être la table déclarée au registre, et
        c'est ce qui fait **tenir P6 par construction** : le nom restauré est *lu*, jamais deviné.
  - [x] rejet de toute substitution de valeur devinée (§5.2, preuve P4)
  - [x] comptage lignes affectées avant/après conservé dans le log — sur le **lot**, pas la table :
        compter la table entière dirait « 20 926 lignes » là où l'humain attend « 351 », et la
        comparaison avant/après perdrait son sens.
  - [x] ⭐ **`BEGIN` explicite plutôt qu'`autocommit(False)`** : chez Snowflake un `BEGIN` ouvre une
        transaction même quand l'autocommit est actif, donc la garantie ne dépend pas d'un réglage de
        session qu'un autre appelant pourrait avoir changé. Si quoi que ce soit lève, on annule — *une
        correction à moitié appliquée serait pire que pas de correction, parce qu'elle serait cohérente
        en apparence.*
  - [x] ⭐ **Les deux premiers garde-fous valent pour le SQL de l'humain aussi** ; seul P6 ne le
        contraint pas. « Ne pas détruire » et « rester dans sa table » protègent de l'**accident**,
        P6 du **jugement** — et l'humain a l'autorité qu'un agent n'a pas.
  - [x] **Un refus n'écrit rien et ne lève pas** : `log` reste la sortie unique, et une exception ferait
        perdre la trace du refus au moment où elle instruit le plus.
- [x] `amend` réel *(2026-08-17)* : la donnée est juste, **le contrat avait tort** → écrit
      `contracts/<dataset>/<table>.v2.yaml`, journalise le diff de clause, **n'écrit rien dans les
      données** (`agent/contracts/amend.py`)
  - [x] ⭐ **Élargir plutôt que retirer, quand c'est possible** : une borne étendue continue de
        protéger contre la valeur suivante, une clause supprimée ne protège plus de rien. *Un
        amendement doit desserrer la règle, pas l'abandonner.*
  - [x] **La v2 est signée d'emblée** : contrairement à la découverte qui propose et attend,
        l'amendement **est** la décision humaine — elle vient d'être prise dans `propose`. La laisser
        en `proposed` obligerait à re-signer ce qu'on vient de trancher, et l'ancienne règle
        continuerait de crier entre-temps.
  - [x] **Un amendement qui n'amende rien n'écrit pas** : une v2 identique à la v1 encombrerait
        l'historique d'une version qui ne dit rien.
  - [x] **Le diff est un livrable, autant que le fichier** — un contrat qui évolue sans trace devient
        au bout de six mois un ensemble de règles que plus personne ne sait justifier, et qu'on n'ose
        donc plus modifier.
...
- [x] `validate` réel *(2026-08-17)* : re-profilage → la métrique anormale est-elle revenue dans la
      ...
      automatique**
  - [x] ⭐ **On re-mesure, on ne croit pas sur parole.** Exécuter une requête sans erreur ne prouve que
        sa syntaxe : un `validate` qui croirait `apply` confirmerait que la requête a tourné, pas
        qu'elle a eu l'effet attendu — *la différence exacte entre « corrigé » et « cru corrigé ».*
  - [x] ⭐ **La comparaison se fait par SIGNATURE**, pas par valeur : ce qu'on veut savoir n'est pas
        « la valeur a-t-elle changé » mais « l'écart existe-t-il encore ». Un seul mécanisme traite
        ainsi les six familles, et il réutilise la signature de 4.4.
  - [x] **Les familles sont appelées directement, pas le nœud `detect`** : celui-ci applique le filtre
        de silence et journalise. Ni l'un ni l'autre n'a de sens ici — *un écart tu par une décision
        passée reste un écart non corrigé.*
  - [x] **Aucune re-tentative automatique** : un agent qui réessaie tout seul écrirait deux fois sans
        qu'un humain ait revu quoi que ce soit, alors même que la première écriture vient d'échouer à
        faire ce qu'elle promettait. Le nom du statut le dit — *manual review*, pas *failed*.

### 5.4 Les tests de preuve (livrables, pas hygiène)
- [x] **P3** : aucun chemin vers `apply` sans `human_decision == "approved"`
...
- [x] **Pause/reprise** : interruption + redémarrage du process + reprise correcte
- [x] **Apply borné** : requête hors table → rejet ; mot-clé destructeur → rejet — et **pour le SQL
      de l'humain aussi** : ces deux règles protègent de l'*accident*, pas du jugement, contrairement
      à P4 qui ne contraint que l'agent
- [x] **Amend n'écrit pas** : après `amend_contract`, aucune ligne de données modifiée — **vérifié par
      comptage avant/après** et non par inspection de l'état : ce qui compte n'est pas qu'`amend`
      *déclare* ne rien écrire, mais qu'aucune ligne n'ait bougé. Seul le fichier de contrat change de
      version.
- [x] ⭐ **Les cinq preuves sont rassemblées dans `tests/test_preuves.py`** *(2026-08-17)*, un fichier
      écrit pour être **lu** : chaque test énonce sa garantie en une phrase et la démontre sur le
      **système assemblé**. `uv run pytest -k preuve` les joue toutes et rien d'autre.
  - [x] **Un troisième angle, pas une redite.** La méthode adoptée en 3.4 établit chaque preuve deux
        fois — *topologiquement* (le graphe compilé, valable pour toute exécution) et *dynamiquement*
        (une exécution sur des décisions hostiles). Ce fichier ajoute le bout-en-bout : les garanties
        tiennent-elles **quand tout est branché ensemble** ? C'est ce qui casserait en premier si une
        phase ultérieure défaisait l'une d'elles sans s'en apercevoir.
  - [x] **Éprouvées par mutation, comme le reste** — une preuve qui ne peut pas échouer ne prouve
        rien, et ce serait d'autant plus grave ici que ce fichier est ce qu'on montre à un jury.
        5 sabotages, 5 détectés : défaut de l'aiguillage vers `apply` (3 échecs), garde-fou P6 retiré
        (1), checkpointer neutralisé (1), bornes d'`apply` sautées (4), `amend` qui écrit aussi dans
        les données (2).
...
### 6.1 Les vues
- [x] **Dashboard BI** : agrégats Gold (ventes par ville/jour…) — l'écran où l'on *voit* les chiffres
      faux avant correction, puis corrigés après
- [x] **Incidents** : historique complet depuis `INCIDENTS` (filtres : couche, table, statut, période)
- [x] **Décision** : pour un incident — anomalie, raisonnement du LLM, cause racine, SQL proposé, impact,
      antécédents
- [x] **Validation HITL** : propositions en pause, diff avant/après estimé, **impact chiffré**, boutons
      **✅ Approuver / 📝 Modifier le contrat / ❌ Refuser** → reprend réellement le graphe interrompu
- [x] **Signatures en silence** ⬅️ *nouveau (2026-07-28)* : la liste de tout ce que l'agent ne signale
...
- [x] **Contrats** : consultation des `contracts/*.yaml` et de leur historique de versions — **et
      validation depuis l'écran** *(2026-08-21, ajout au plan)* : `agent/contracts/validation.py`
      partage `approuver()` avec `scripts/discover.py --approve`. Une seconde voie de signature serait
      une seconde façon de contourner `--accept-warnings`, et la critique de la découverte deviendrait
      décorative. Le garde-fou survit tel quel dans l'interface : une **case à cocher** « j'ai lu les
      N réserves », et le bouton reste gris tant qu'elle ne l'est pas.
- [x] ⭐ **Toute la logique dans `streamlit/donnees.py`, l'affichage seul dans `app.py`** *(2026-08-17)*
      — même partage qu'entre le DAG et `scripts/check_layer.py`. Ce n'est pas une préférence de
      style : une logique enfermée dans un `st.button()` n'est éprouvable qu'à la main, et *un écran
      de décision qu'on ne peut pas tester est un écran dont on ne sait pas s'il montre la vérité*.
- [x] ⭐ **`agent/hitl.py` — la voie UNIQUE de reprise**, extraite de `scripts/decide.py` : les boutons
      et la ligne de commande passent exactement par le même code. Une seconde voie serait une
      **seconde façon de contourner P3**, et la garantie « aucun chemin n'atteint `apply` sans
      approbation » ne vaudrait plus que pour les chemins qu'on a testés. Un test relit la couche de
      données pour vérifier qu'aucun raccourci « pratique » n'y est apparu.
  - [x] **Le module ne parle à personne** : aucun `print`, aucun composant d'interface — il rend des
        dictionnaires, et une **erreur est une donnée** (`{"ok": False, "code", "erreur"}`). Le code
        accompagne la phrase parce que les deux interfaces n'ont pas le même vocabulaire : le terminal
        veut dire « utilisez `--fix` », le navigateur veut **désactiver un bouton**.
  - [x] **Vérifié avant d'injecter qu'une pause existe vraiment** : `Command(resume=…)` sur un fil qui
        n'attend rien relancerait le graphe **depuis le début**, et l'humain croirait avoir tranché
        une proposition qui n'existait plus.
- [x] ⭐ **Le journal n'est pas la mémoire** — `lire_journal()` ajouté à côté de `lire_incidents()` :
      la seconde filtre aux décisions humaines (R5) parce que c'est ce que l'**agent** relit ; la
      première ne filtre rien parce que c'est ce qu'un **humain** relit. Cacher les runs sans anomalie
      donnerait un historique **plus propre que la réalité**, et c'est précisément là qu'on regarde
      pour savoir ce que l'agent a fait cette nuit.
- [x] **Le statut d'un incident est dérivé, jamais stocké** : « en attente » n'est pas une valeur de
      `human_decision`, c'est son absence. Un champ stocké pourrait mentir — un incident tranché resté
      « en attente » serait invisible pour toujours.
- [x] ⭐ **Refonte pour des yeux non techniques** *(2026-08-21, à la demande)* — la personne qui décide
      sait si les ventes par ville sont justes ; elle ne sait pas ce qu'est « un écart de famille
      sémantique d'octave −2 », et elle n'a pas à l'apprendre pour trancher.
  - [x] **Un écran d'accueil qui répond à *que dois-je faire ?***, pas seulement à *comment ça va ?* :
        quatre chiffres, puis « ce qui vous attend ». Un tableau de bord contemplatif ne sert à rien.
  - [x] **Tout le vocabulaire traduit dans `donnees.py`** — à un seul endroit, donc corrigeable d'un
        seul geste : `{"between": [1, 100]}` devient « doit rester entre 1 et 100 », `approved`
        devient « ✅ corrigé », « signatures en silence » devient « alertes désactivées ». Les
        **chiffres** sont gardés : « des valeurs manquent » sans « 51 lignes » ne permet pas de
        décider.
  - [x] **Les trois réponses parlent métier** : *Corriger* (la donnée est fausse), *Ajuster la règle*
        (la donnée est juste), *Écarter* (cas isolé) — au lieu d'approuver / amender / refuser.
  - [x] Les six écrans réorganisés en trois questions : **où en est-on** (Accueil) · **que dois-je
        faire** (Décisions, Règles) · **que s'est-il passé** (Données, Historique, Alertes désactivées).
- [x] ⚠️ **Collision de noms assumée** : le dossier s'appelle `streamlit/`, comme le paquet installé.
      `from streamlit import donnees` résout vers `site-packages` et échoue. On importe le module nu,
      exactement comme le fait `streamlit run` — le dossier garde son nom, écrit dans la structure du
      dépôt depuis la phase 0.

### 6.2 Intégration & démo
- [x] Le clic Approuver/Refuser passe par le même mécanisme que `scripts/decide.py` (une seule voie de
      reprise, **testée**) *(2026-08-21)* — `agent/hitl.py`, et `tests/test_streamlit_app.py` exécute
      l'application **sans navigateur** (`streamlit.testing.v1.AppTest`) pour le vérifier au lieu de le
      supposer. Un bouton qui appellerait un raccourci maison serait une seconde façon de contourner
      P3, et **personne ne le verrait avant la démo**.
- [x] Identité du validateur recueillie (champ nom) → `decided_by` — les trois boutons sont
      **désactivés** tant qu'il est vide, et non refusés après coup : *apprendre un refus après avoir
      cliqué n'apprend rien*. Une décision sans auteur ne se conteste pas six mois plus tard, et le
      taux d'approbation de la phase 8 n'aurait personne à qui l'attribuer.
- [ ] ⏭️ Rejouer le **fil rouge complet à la souris** : casser (injection) → détecter → proposer →
      approuver → corriger → vérifier → journal — sans terminal. **Runbook écrit**
      ([`docs/DEMO.md`](docs/DEMO.md)), enchaînement **à jouer sur le PC** avec Snowflake actif : les
      six écrans sont testés isolément, leur enchaînement ne l'est pas. À grouper avec 4.5/4.6/5.5.
...
| 2026-08-17 | 5 | ✅ **5.3 — `apply` borné, `amend` réel, `validate` réel** : écriture transactionnelle dans le connecteur, l'unique exception DDL, `agent/contracts/amend.py`, re-profilage par signature. **+18 tests, 659 verts.** 7 sabotages joués, **7/7 détectés**. | **⭐ L'autorisation DDL ne se donne pas parce que le SQL en a la forme, mais parce qu'un FAIT constaté l'appelle.** C'est la décision de conception de l'étape : sans la condition « un écart de famille `inventaire` existe », il suffirait au modèle d'écrire un renommage de table pour franchir un garde-fou qui existe précisément pour l'en empêcher. La cible est vérifiée en plus — elle doit être la table déclarée au registre — et c'est ce qui fait **tenir P6 par construction** : le nom restauré est *lu*, jamais deviné. **⭐ `BEGIN` explicite plutôt qu'`autocommit(False)`** : chez Snowflake un `BEGIN` ouvre une transaction même quand l'autocommit est actif, donc la garantie ne dépend pas d'un réglage de session qu'un autre appelant pourrait avoir changé. *Une correction à moitié appliquée serait pire que pas de correction, parce qu'elle serait cohérente en apparence.* **Comptage sur le lot, pas la table** : compter la table entière dirait « 20 926 lignes » là où l'humain attend « 351 ». **⭐ `amend` élargit plutôt que retire** : une borne étendue continue de protéger contre la valeur suivante, une clause supprimée ne protège plus de rien. *Un amendement doit desserrer la règle, pas l'abandonner.* Et la v2 est **signée d'emblée** — contrairement à la découverte qui propose et attend, l'amendement *est* la décision humaine, qui vient d'être prise dans `propose`. **⭐ `validate` re-mesure au lieu de croire `apply` sur parole** : exécuter une requête sans erreur ne prouve que sa syntaxe. La comparaison se fait **par signature** et non par valeur — ce qu'on veut savoir n'est pas « la valeur a-t-elle changé » mais « l'écart existe-t-il encore » —, ce qui traite les six familles d'un seul mécanisme, réutilisé de 4.4. Les familles sont appelées **directement**, pas le nœud `detect` : celui-ci filtre les signatures tues, or *un écart tu par une décision passée reste un écart non corrigé*. **⚠️ Trou de test découvert et refermé** : les tests de reprise lancent de vrais sous-processus, et n'y installaient que les doubles **qu'on avait remarqués**. `diagnose` n'en avait aucun — ces sous-processus appelaient donc le **vrai Groq** à chaque exécution de la suite, en silence. C'est l'incident de la phase 3.3 reproduit par un chemin que les fixtures ne couvrent pas, et c'est la même leçon une troisième fois : *une règle qu'on peut oublier n'est pas une règle.* Elle vit désormais dans `tests/sitecustomize.py`, que Python importe au démarrage de tout interpréteur recevant le dossier des tests sur son `PYTHONPATH`. **Preuve chiffrée** : la suite est passée de 18 s à 15 s en le refermant. **Second piège du même genre** : `validate` importe **sa propre** référence à `profile_table`, donc patcher celle de `profile` ne suffisait pas — un `monkeypatch` qui vise le mauvais objet ne remplace rien, silencieusement. C'est la barrière anti-connexion de 4.3 qui l'a attrapé. **Incohérence de format corrigée** : le numéro de version du contrat est un **entier** dans le fichier (4.2.4) mais traînait en `"v1"` dans l'état depuis le stub de la phase 3 — `amend` réel l'aurait fait lever au premier amendement réel. |
...
| 2026-08-21 | 6 | ✅ **Refonte de l'interface pour des non-techniciens + validation d'un contrat depuis l'écran** (à la demande) : `agent/contracts/validation.py`, écran d'accueil, traduction complète du vocabulaire. **+11 tests, 721 verts.** 5 sabotages joués : **3/5 au premier tour**, 5/5 après correction. | **⭐ Le vrai défaut de la première version n'était pas esthétique.** Les écrans affichaient le vocabulaire du projet — « écarts », « familles », « signatures en silence », `{"between": [1, 100]}` — à quelqu'un qui connaît le **métier** et pas le projet. Un contrat est précisément la chose qu'un métier doit pouvoir relire **avant de la signer** : `{"between": [1, 100]}` ne se relit pas, « doit rester entre 1 et 100 » se relit. La traduction vit dans `donnees.py`, donc à un seul endroit et donc testable — un mot mal choisi se corrige d'un geste. **Les chiffres sont gardés** : « des valeurs manquent » sans « 51 lignes » ne permet à personne de décider. **Un écran d'accueil qui répond à *que dois-je faire ?***, et non seulement à *comment ça va ?* — quatre chiffres, puis « ce qui vous attend », avec le lien vers l'écran d'action. Un tableau de bord contemplatif est un tableau de bord qu'on cesse d'ouvrir. **⭐ Validation d'un contrat depuis l'interface — et le garde-fou survit.** `approuver()` est extrait de `scripts/discover.py` vers `agent/contracts/validation.py` : même raisonnement que pour `agent/hitl.py` en 6.1, une seconde voie serait une seconde façon de contourner `--accept-warnings`. Dans l'écran, ce garde-fou devient une **case à cocher** « j'ai lu les N réserves », et le bouton reste gris tant qu'elle ne l'est pas — *signer une collision sémantique est une décision, pas une formalité*. **Leçon de méthode — deux garde-fous non testés, encore.** Le sabotage « signer n'exige plus de nom » passait inaperçu : la règle vivait dans `validation.py` mais **aucun test ne l'y éprouvait**, seulement le `argparse` de `discover.py` le vérifiait de son côté. Quatrième occurrence du même motif (4.2.5, 6.1, 6.2, ici) — *une règle vérifiée par l'interface et non par le module qu'elle protège n'est pas une règle, c'est une habitude*. **Incident d'outillage, ma faute** : un `git checkout` malheureux pendant une restauration de mutation a effacé la couche de traduction que je venais d'écrire. Rattrapé, mais la leçon est nette — **ne jamais mêler `git checkout` et une sauvegarde de fichier** dans un même script de restauration. |
